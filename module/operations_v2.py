...
def install_operations(app: Any, lar: Any) -> OperationsRuntime:
    """Register operational routes for the loopback-only console."""
    ...
    async def cleanup_preview(payload: dict[str, Any] = Body(default={})):
        result = runtime.cleanup_candidates(payload)
        result.pop("_all", None)
        return result

    @router.post("/api/operations/cleanup/run")
    async def cleanup_run(payload: dict[str, Any] = Body(...)):
    ...
    async def restore_backup(name: str, payload: dict[str, Any] = Body(...)):
    ...
    async def get_rule(channel_id: str):
        return runtime.settings.get("rules", {}).get(channel_id, {})

    @router.put("/api/operations/rules/{channel_id}")
    async def put_rule(channel_id: str, payload: dict[str, Any] = Body(...)):
