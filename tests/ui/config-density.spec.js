...
test('settings workspace reduces the page to five focused groups', async ({ page, viewport }, testInfo) => {
  ...
  await expect(page.getByRole('tab', { name: /기본 녹화/ })).toHaveAttribute('aria-selected', 'true');
  ...
  await page.getByRole('tab', { name: /치지직/ }).click();
  ...
  await page.getByRole('tab', { name: /시스템·보안/ }).click();
