(function () {
  ...
  function renderHealth() {
    ...
    if (form && cfg && !form.dataset.filled) {
      ["stall_seconds", "max_restart_attempts", "restart_cooldown_seconds"].forEach(function (name) { form.elements[name].value = cfg[name] ?? ""; });
  ...
  async function loadAll() {
    ...
    Object.assign(state, { summary, settings, health: health.channels || [], jobs: jobs.jobs || [], backups: backups.backups || [], statistics, audit: audit.entries || [] });
  ...
  function bindForms() {
    document.getElementById("ops-storage-form")?.addEventListener("submit", async function (event) { event.preventDefault(); const f = event.currentTarget.elements; try { await saveSection("storage", { warning_free_percent: Number(f.warning_free_percent.value), block_free_percent: Number(f.block_free_percent.value), cleanup_mode: f.cleanup_mode.value, retention_days: Number(f.retention_days.value), max_total_gb: Number(f.max_total_gb.value), auto_cleanup: f.auto_cleanup.checked }); await loadAll(); } catch (error) { notice(error.message, "error"); } });
    document.getElementById("ops-health-form")?.addEventListener("submit", async function (event) { event.preventDefault(); const f = event.currentTarget.elements; try { await saveSection("health", { stall_seconds: Number(f.stall_seconds.value), max_restart_attempts: Number(f.max_restart_attempts.value), restart_cooldown_seconds: Number(f.restart_cooldown_seconds.value), auto_restart: f.auto_restart.checked, enabled: true }); await loadAll(); } catch (error) { notice(error.message, "error"); } });
  ...
  function bindActions() {
    ...
    document.addEventListener("click", async function (event) {
      const retry = event.target.closest("[data-job-retry]"); const cancel = event.target.closest("[data-job-cancel]"); const restore = event.target.closest("[data-backup-restore]");
      try {
